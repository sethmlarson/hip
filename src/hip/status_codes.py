...
class StatusCode(int, enum.Enum):
    """Status code enum for doing comparisons to Response.status_code.
    Status codes for WebDAV which have no specific semantics
    for HTTP are marked.

    This list was pulled from the IANA HTTP Status Code Registry:
    https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml
    with the exception of 418 which is *technically* unassigned but ya'know. :-)
    """
